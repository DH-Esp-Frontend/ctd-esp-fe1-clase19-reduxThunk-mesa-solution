...
export const thunkGetPokemons = createAsyncThunk("pokemons/",async ()=>{
  const response = await fetch("https://pokeapi.co/api/v2/pokemon?limit=151");
  const data = await response.json();
  return data.results;
})

export const thunkSearchPokemon = createAsyncThunk("pokemons",async (pokemonName)=>{
...
export const thunkGetAnPokemon = createAsyncThunk("pokemons",async(pokemonName)=>{
